...
final_game_data= left_join(playmydata_pc_data, steam_game_data, by = "name")

#removing na values
filtered_game_data= final_game_data|>
  filter(!is.na(app_id))   #the resulting dataset has only 30,000 games

#Removing games with missing ratings and completion times
filtered_game_data= filtered_game_data|>filter(!rating%in%c("Missing","nan"))
filtered_game_data = filtered_game_data|> filter(!main=="NaN")
#arranging for easier view
filtered_game_data|> arrange(rating)
